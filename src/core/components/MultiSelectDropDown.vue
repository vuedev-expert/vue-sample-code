...
import { MutltiSelectDropDownItem } from '../models/common.model.js'

const props = defineProps({
  items: {
    type: Array<MutltiSelectDropDownItem>,
    default: []
  },
  ...
  shownInModal: { type: Boolean, default: false }
})
...
<template>
  <Menu as="div" class="relative inline-block text-left bg-base-white w-full">
    <MenuButton :disabled="props.disabled" class="w-full">
      <div
        class="inline-flex w-full h-10 items-center rounded-sm transition-all duration-200 bg-base-white pl-4 pr-3 py-2 border border-gray-400 focus:outline-none justify-between"
    ...
    >
      <MenuItems
        :style="[
            !props.wFull ? { width: props.width + 'px' } : { width: '100%' },
            props.shownInModal ? { maxHeight: '150px' } : { maxHeight: 'calc(100vh - 400px)' },
        ]"
        class="absolute overflow-y-visible z-[999] bg-base-white py-2 left-0 mt-2 origin-top-left rounded-md bg-white shadow-lg focus:outline-none filter-dropdown"
      >
        <div v-if="props.hasSearch" class="px-2 py-2">
          <input v-model="search" type="text" placeholder="Search" class="form-input search-dropdown pr-8" />
        </div>
