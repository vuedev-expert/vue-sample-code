<template>
  <div class="flex-1 max-w-md">
    ...
    <div class="bg-base-white shadow-md rounded-lg relative p-10 overflow-hidden">
      <div class="text-2xl text-primary-500 font-semi-bold uppercase mb-8 text-center">Admin Sign In</div>
      <form @submit="submitForm">
        ...
        <div class="w-full text-end mb-8">
          <router-link
              id="forgotPass"
              :to="{ name: AdminRouteNames.ADMIN_FORGOT_PASSWORD }"
...
import { AdminRouteNames } from '@/core/models/app-router.model'
...
const submitForm = (event?: Event) => {
  ...
  v$.value.$validate()
  if (!v$.value.$invalid) {
    authStore.adminSignIn(state.value)
  }
}
